...
class MLAnalyzer:
    ...
    def perform_pca(self, columns=None, n_components=None):
        ...
        Args:
            ...
            n_components (int, optional): Number of principal components to compute.
                If None, min(n_samples, n-features) will be used.
        ...
        if df is not None:
            ...
            if len(columns) >= 2:
                ...
                if len(pca_data) > 0:
                    ...
                    # Datermine number of components
    ...
    def create_pca_plots(self,pca_result):
        ...
        if pca_result is not None:
            ...
            ax_var.set_xlabel('Principle Component')
            ...
            if pca_result['pca_result'].shape[1] >= 2:
                ...
                ax_scatter.set_title('PCA: First Two Principle Components')
                ax_scatter.set_xlabel(f'PC1({pca_result["explained_variance"][0]:.2f}%)')
                ax_scatter.set_ylabel(f'PC1({pca_result["explained_variance"][1]:.2f}%)')
                plots['scatter'] = fig_scatter


            # Feature importance for PC1
            fig_imp, ax_imp = plt.subplots(figsize=(10,6))
            loading = pca_result['loadings'].copy()
            loading.sort_values(by='PC1', key=abs,ascending=False)['PC1'].plot(kind='bar', ax=ax_imp)
            ax_imp.set_title('Featuer Importance for PC1')
