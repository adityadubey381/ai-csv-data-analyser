...
class Visualizer:
    """
    A class for creating various data visualization.
    """
    ...
    def create_distribution_plots(self, column):
        ...
        if df is not None and column in df.columns:
            ...
            sns.barplot(y= df[column].dropna(),ax=box_ax)
    ...
    def create_correlation_heatmap(self, columns=None):
        ...
        if df is not None:
            if columns is None:
                columns = self.data_loader.get_numeric_coulmns()
    ...
    def create_categorical_pot(self, column):
        """
    ...
    def create_group_by_plot(self,group_column, agg_column, agg_func='mean'):
        ...
        if df is not None and group_column in df.columns and agg_column in df.columns:
            ...
            # Use the map function or the original if not in the map.
            func = agg_map.get(agg_func,agg_func.lower())

            # Perform group by opeation
    ...
    def create_pairplot(self, columns=None, hue=None, n_samples=1000):
        ...
        if df is not None:
            if columns is None:
                columns = df.data_loader.get_numeric_columns()
                # Limit to 5 columns to avoid exessive computation
            ...
            if len(columns) >= 2:
                ...
                pairplot.fig.subtitle("Pairwise Relationships", y=1.02)
                plt.tight_layout()
    ...
    def create_time_series_plot(self, date_column, value_column, freq=None):
        ...
        if df is not None and date_column in df.columns and value_column in df.columns:
            try:
                ...
                # Set date as index
                ts_df = df.set_indext(date_column)
